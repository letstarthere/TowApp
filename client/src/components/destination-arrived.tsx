...
interface DestinationArrivedProps {
  onComplete: () => void;
}

export default function DestinationArrived({ onComplete }: DestinationArrivedProps) {
  const [currentMessageIndex, setCurrentMessageIndex] = useState(0);
  const [isVisible, setIsVisible] = useState(true);

  const messages = [
    "The driver is preparing to unload your vehicle",
    "The driver is carefully removing your vehicle from the truck",
    "Final inspection and handover in progress"
  ];
  ...
  return (
    <div className="bg-white p-6 flex flex-col items-center justify-center text-center" style={{ height: '40vh' }}>
      <h2 className="text-2xl font-bold text-black mb-6">Destination Arrived</h2>
